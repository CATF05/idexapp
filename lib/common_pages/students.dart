...
import 'package:frontend/data_sources/student_data_source.dart';
...
class StudentsScreen extends StatelessWidget {
  Widget home;
  StudentsScreen({super.key, required this.home});
...
class StudentsScreenHome extends StatefulWidget {
  Widget home;
  StudentsScreenHome({super.key, required this.home});

  @override
  State<StudentsScreenHome> createState() => _StudentsScreenHomeState();
}

class _StudentsScreenHomeState extends State<StudentsScreenHome> {
  TextEditingController searchController = TextEditingController();
  bool _sortAscending = true;
  int? _sortColumnIndex;
  late StudentDataSource _studentsDataSource;
  bool _initialized = false;
  bool showCustomArrow = false;
  bool sortArrowsAlwaysVisible = false;
  List<EtudiantModel> initialEtudiants = [];
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        for (var f in querySnapshot.docs) {
          ...
          initialEtudiants.add(EtudiantModel.fromMap(f.data()));
        }
      }
      setState(() {});
    } catch (e) {
  ...
  void didChangeDependencies() {
    ...
    if (!_initialized) {
      ...
      _studentsDataSource = StudentDataSource(
          context,
          widget.home,
          etudiants,
          false,
      ...
      _initialized = true;
      _studentsDataSource.addListener(() {
        setState(() {});
      });
    }
  }
  ...
  void _sort<T>(
    Comparable<T> Function(EtudiantModel d) getField,
    int columnIndex,
    bool ascending,
  ) {
    _studentsDataSource.sort<T>(getField, ascending);
    setState(() {
                ...
                Theme(
                  ...
                  child: SizedBox(
                    ...
                    child: DataTable2(
                      ...
                      columnSpacing: 12,
                      horizontalMargin: 12,
                      sortArrowBuilder: getCurrentRouteOption(context) ==
                              custArrows
                      ...
                      onSelectAll: (val) =>
                          setState(() => _studentsDataSource.selectAll(val)),
                      columns: [
                        DataColumn2(
                          label: const Text('Identifiant'),
                          size: ColumnSize.S,
                          onSort: (columnIndex, ascending) => _sort<String>(
                              (d) => d.idStudent, columnIndex, ascending),
                        ),
                        DataColumn2(
                          label: const Text('Prénom'),
                          size: ColumnSize.S,
                          onSort: (columnIndex, ascending) => _sort<String>(
                              (d) => d.prenom, columnIndex, ascending),
                        ),
                        DataColumn2(
                          label: const Text('Nom'),
                          size: ColumnSize.S,
                          onSort: (columnIndex, ascending) => _sort<String>(
                              (d) => d.nom, columnIndex, ascending),
                        ),
                        DataColumn2(
                      ...
                      rows: getCurrentRouteOption(context) == noData
                          ? []
                          : List<DataRow>.generate(_studentsDataSource.rowCount,
                              (index) => _studentsDataSource.getRow(index)),
                    ),
                  ),
                ),
              ])),
